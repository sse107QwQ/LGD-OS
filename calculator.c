...
int cmd_calc(ShellContext *ctx, int argc, char *argv[], FILE *in, FILE *out) {
    ...
    if (argc < 2) {
        ...
        fprintf(out, "\n");
        fprintf(out, "多元一次方程组模式:\n");
        fprintf(out, "  格式: 方程1;方程2;方程3;...\n");
        fprintf(out, "  示例: 2x1+3x2=8;4x1+x2=6\n");
        fprintf(out, "        x1+x2+x3=6;2x1+x2+3x3=13;x1+2x2+x3=8\n");
        fprintf(out, "  注意: 变量必须为x1,x2,x3,...形式\n");
        fprintf(out, "        系数必须明确写出，但如果是1不需要\n");
        fprintf(out, "        方程用分号分隔\n");
        fprintf(out, "\n");
        fprintf(out, "示例:\n");
        ...
        fprintf(out, "  calc -S \"2x1+3x2=8;4x1+x2=6\"  # 二元一次方程组\n");
        return 0;
    }
