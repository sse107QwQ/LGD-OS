...
int cmd_run(ShellContext *ctx, int argc, char *argv[], FILE *in, FILE *out) {
    ...
    if (argc < 2) {
        fprintf(out, "\n用法: run <可执行文件> [参数]\n");
        fprintf(out, "示例: run notepad.exe\n");
        fprintf(out, "      run /bin/helloworld.exe\n");
        fprintf(out, "      run /home/user/program.exe\n");
        return 1;
    }
    ...
    // 获取当前虚拟工作目录
    char virtual_current_dir[MAX_PATH_LEN];
    if (ctx->current_dir[0] == '/') {
        strcpy(virtual_current_dir, ctx->current_dir);
    } else {
        snprintf(virtual_current_dir, sizeof(virtual_current_dir), "/%s", ctx->current_dir);
    }

    fprintf(out, "\n虚拟工作目录: %s\n", virtual_current_dir);
    fprintf(out, "要执行的程序: %s\n", exe_name);
        ...
        if (f) {
            ...
            if (!f) {
                ...
                if (last_slash) {
                    ...
                    strncpy(real_dir, real_path, dir_len);
                    real_dir[dir_len] = '\0';
                } else {
                    getcwd(real_dir, sizeof(real_dir));
                }

                snprintf(cmd, sizeof(cmd), "dir \"%s\" 2>nul", real_dir);
                system(cmd);
    ...
    if (last_slash) {
        ...
        strncpy(real_dir, real_path, dir_len);
        real_dir[dir_len] = '\0';
        strcpy(real_file, last_slash + 1);
    } else {
        strcpy(real_file, real_path);
    }

    if (strlen(real_dir) > 0) {
        fprintf(out, "实际目录: %s\n", real_dir);
    }
    ...
    if (strlen(real_dir) > 0) {
        // 切换到实际目录执行
        if (strchr(real_dir, ' ') != NULL) {
            // 目录包含空格，用引号
            if (strchr(real_file, ' ') != NULL) {
                // 文件名也包含空格
                snprintf(command, sizeof(command),
                        "cd /d \"%s\" && \"%s\"", real_dir, real_file);
            } else {
                snprintf(command, sizeof(command),
                        "cd /d \"%s\" && %s", real_dir, real_file);
            }
        ...
        if (strchr(real_path, ' ') != NULL) {
            snprintf(command, sizeof(command), "\"%s\"", real_path);
        } else {
            snprintf(command, sizeof(command), "%s", real_path);
        }
    }
