...
void shell_display_prompt(ShellContext *ctx) {
    ...
    if (last_slash != NULL) {
        // 移动到下一个字符（跳过 '/'）
        display_dir = last_slash + 1;

        // 如果结果是空字符串（如 "/path/to/"），回退到完整路径
        if (*display_dir == '\0') {
    ...
    // 精确颜色控制：
    // 用户名@主机名（绿色）: [（无色）目录名（蓝色）（无色）] $（无色）
    printf("\033[32m%s@%s\033[0m:[\033[34m%s\033[0m]$ ",
           username, hostname, display_dir);
    fflush(stdout);
}
