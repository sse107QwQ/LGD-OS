...
int cmd_help(ShellContext *ctx, int argc, char *argv[], FILE *in, FILE *out) {
    ...
    if (argc == 1) {
        ...
        // 限制列宽，避免过宽
        if (column_width > 35) column_width = 35;

        // 按列显示命令
        int cols = 3;
        int rows = (command_count + cols - 1) / cols;

        // 计算实际列数（避免最后一列只有1-2个命令）
        int actual_cols = 3;
        for (actual_cols = 3; actual_cols > 1; actual_cols--) {
            int actual_rows = (command_count + actual_cols - 1) / actual_cols;
            int last_col_count = command_count - (actual_cols - 1) * actual_rows;
            if (last_col_count >= 2) break; // 最后一列至少有2个命令
        }

        cols = actual_cols;
        rows = (command_count + cols - 1) / cols;
            ...
            for (int col = 0; col < cols; col++) {
                ...
                if (idx < command_count) {
                    ...
                    snprintf(display_str, sizeof(display_str), "%-*s - %s",
                            max_name_len, commands[idx].name, commands[idx].brief);
